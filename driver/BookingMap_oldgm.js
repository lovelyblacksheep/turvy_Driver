...
export default class BookingMap extends Component {
    ...
    handleMapBoxState = async () => {
        console.log('call from NavigationMapbox child componant')
        this.setState({
            mapbox: false
        })
    }
    ...
    changeMode = async () =>{
        ...
        this.intervalModeChange = setInterval(async () => {            
            ...
            var dt = new Date();
            var newDarkMode =  (Moment(dt).format('HH') >= 18  || (Moment(dt).format('HH') < 6  || (Moment(dt).format('HH') == 6 && Moment(dt).format('mm') <=30))) ? true : false;
            console.log("componentDidMount DATE TIME :::: " + new Date() + "::" + Moment(dt).format('HH')  +"::" +  Moment(dt).format('mm')+"::" +  this.state.isDarkMode +":::"+newDarkMode )
    ...
    _navigationBannar = async (destination) => {
        this.intervalBanner = setInterval(async () => {            
            ...
            this.state.origin // Pickup
            this.state.destination // DESTINATION 

            if(this.state.driverLocation && destination)
            {
                let url = "";
                if(this.state.destination.latitude == destination.latitude)
                url = `https://maps.googleapis.com/maps/api/directions/json?origin=${this.state.driverLocation.latitude},${this.state.driverLocation.longitude}&destination=${this.state.destination.latitude},${this.state.destination.longitude}&mode=driving&avoid=tolls|ferries&key=AIzaSyAr2ESV30J7hFsOJ5Qjaa96u9ADSRX6_Sk`;
            else
                url = `https://maps.googleapis.com/maps/api/directions/json?origin=${this.state.driverLocation.latitude},${this.state.driverLocation.longitude}&waypoints=${this.state.origin.latitude},${this.state.origin.longitude}&destination=${this.state.destination.latitude},${this.state.destination.longitude}&mode=driving&avoid=tolls|ferries&key=AIzaSyAr2ESV30J7hFsOJ5Qjaa96u9ADSRX6_Sk`;
...
//             }
    ...
    render() {
        ...
        return (
                ...
                this.state.mapbox
                ?
                <NavigationMapbox origin={this.state.mapboxSrc} dest={this.state.mapboxDst} book_id={this.state.book_id} handleMapBoxState = {this.handleMapBoxState} isDeparted={this.state.departed} handleTravlDist = {this.handleTravlDist} />
                :
            <>

               {
                ...
                ?
                <View style={{position:'absolute', zIndex: 100, top:'0%',left:0, right: 0, width: '100%', alignItems:'center', height: 'auto', backgroundColor: 'transparent' }}>
                    
                        <View style={{width:'100%',flexDirection:'row',alignItems:'center', justifyContent:'space-between', backgroundColor: "#135aa8",borderRadius: moderateScale(10), paddingHorizontal: moderateScale(5),paddingTop: moderateScale(20),paddingBottom:moderateScale(5)}}>
                            
                                <Image resizeMode='contain' source={this.state.bannerIcon} style={{width:moderateScale(50),height:moderateScale(50)}} />
                           
                             <View style={{alignItems:'flex-start', width: '90%' }}>
                                <Text style={{fontSize:moderateScale(12),fontWeight:'bold',color:'#fff'}}>{this.state.bannerDistance}</Text>
                                <Text style={{color:'#fff', fontWeight:'bold',numberOfLines:2, fontSize: moderateScale(22)}}>{this.state.bannerText}</Text>
                            </View>
                        </View>
                    
                    {  this.state.bannerDistanceNext ?
                     <View style={{ marginTop:-8,
                         marginStart:'0%',  alignItems:'flex-start',flexDirection:'row',alignSelf:'flex-start', backgroundColor: "#135aa8",borderBottomLeftRadius: moderateScale(10), borderBottomRightRadius: moderateScale(10), paddingHorizontal: moderateScale(15),paddingVertical: moderateScale(10)}}>
